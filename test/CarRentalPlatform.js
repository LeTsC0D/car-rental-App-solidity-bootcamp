...
describe("Check out and check in car", () => {
  ...
  assert.equal(user.debt,10,"User did not get created");
...
describe("Deposit token and make payment", () => {
  ...
  await carRentalPlatform.addUser("Alice", "Smith", { from: user1 });
  await carRentalPlatform.addCar("Tesla Model S", "example url", 19, 50090, { from: owner });
  await carRentalPlatform.checkOut(1, { from: user1 });
  await new Promise((resolve) => setTimeout(resolve, 66960)); // 1 min
  await carRentalPlatform.checkIn({ from: user1 });
...
describe("edit car", () => {
  it("shou1d edit an existing car‘s metadata with valid parameters", async () => {
  await carRentalPlatform.addCar("Tesla Model 5", "example img url", 10, 50000, { from: owner });
  const newName = "Honda";
  ...
  it("shou1d edit an existing car's status", async () => {
  ...
  describe("w1thdraw balance", () => {
    it("shou1d send the desired amount of tokens to the user", async () => {
    await carRentalPlatform.addUser("Alice", "Smith", { from: user1 });
    await carRentalPlatform.deposit({ from: user1, value: 100 });
    ...
    it("shou1d send the desired amount of tokens to the owner", async () => {
      await carRentalPlatform.addUser("Alice", "Smith", { from: user1 });
      await carRentalPlatform.addCar("Tesla Model S", "example img url", 10, 50000, { from: owner });
      await carRentalPlatform.checkOut(1, { from: user1 });
      await new Promise((resolve) => setTimeout(resolve, 60000)); // 1 min
